...
extension OpenAISwift {
    ...
    public func sendCompletion(with prompt: String, model: OpenAIModelType = .gpt3(.davinci), maxTokens: Int = 16, temperature: Double = 1, logprobs: Int = 5, completionHandler: @escaping (Result<OpenAI, OpenAIError>) -> Void) {
        let endpoint = Endpoint.completions
        let body = Command(prompt: prompt, model: model.modelName, maxTokens: maxTokens, temperature: temperature, logprobs: logprobs)
        ...
        makeRequest(request: request) { result in
            ...
            case .success(let success):
                do {
                    ...
                    //print(String(data:try JSONEncoder().encode(success), encoding: .utf8) ?? "")
...
extension OpenAISwift {
    ...
    public func sendCompletion(with prompt: String, model: OpenAIModelType = .gpt3(.davinci), maxTokens: Int = 16, temperature: Double = 1, logprobs: Int = 0) async throws -> OpenAI {
        return try await withCheckedThrowingContinuation { continuation in
            sendCompletion(with: prompt, model: model, maxTokens: maxTokens, temperature: temperature, logprobs:logprobs) { result in
